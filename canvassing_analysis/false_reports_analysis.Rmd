...
data_raw <- data_raw %>%
  ...
  mutate(type_report = str_replace_all(type_report, "Corporate Collaboration", "Corp Collab"),
         ...
         type_report = str_replace_all(location, "Chicago - ", ""),
...
When we look at cases where a rapid responder was able to be respond (X/1290 cases), we can see that:  

```{r}

```

In terms of geographic distribution of false sightings, false reports across all categories are concentrated in the following neighborhoods. 

```{r}
data_location <- data_labeled %>%
...
1. ICE Sightings are by far the common type of the. Thus far, the network has recorded X confirmed ICE Sightings, X Public Space Raids, Y Home Raids, and Z I-9 raids. 

Note that the majority of Public Space Raids have been concentrated around X (). 

2. In cases where a responder was dispatched (verified by RRT) to an Ice Sighting, the responder was only able to verified as X% as true and Y% as false. The majority of cases (X%) were unable to classified by a responder. 

When pressed for capacity, would highly recommend responders *triage alleged raids over ICE sightings.* 
