...
ggplot(data_labeled, aes(x = refined_categories)) +
   ...
   labs(title="Histogram Classifying Types of Incidents (All Reports)", 
       subtitle="Includes data from 2.1.2025 to 9.28.25") +
...
Some notable exceptions exist. For example, false incidents comprise **11.75** of total ICE sightings, but where a rapid responder was dispatched, incidents were proven false **22.3%** of the time (*note that a two proportion z-test confirms this gap is significant, p <.001).

This gap likely occurs because the Airtable database contains additional confirmed incidents that do not come from the FSN. 
...
evanston_boundaries <- st_read("https://maps.cityofevanston.org/arcgis/rest/services/OpenData/ArcGISOpenData/MapServer/0/query?where=1=1&outFields=*&f=geojson", quiet = TRUE) %>%
...
cicero_berwyn_boundaries <- st_read("https://geoservices.epa.illinois.gov/arcgis/rest/services/Political/IllinoisPoliticalBoundaries/MapServer/2/query?where=DIST_NAME+IN+('Cicero','Berwyn')&outFields=*&f=geojson", quiet = TRUE) %>%
...
# Label neighborhoods to correspond with Chicago community areas penis54444
location_data <- data_labeled %>%
  mutate(community = str_to_upper(location),
         ...
         community = case_when(
                    ...
                    str_detect(community, "LITTLE VILLAGE|LA VILLITA|MARSHALL SQUARE/HEART OF CHICAGO	") ~ "SOUTH LAWNDALE",
                    ...
                    str_detect(community, "BRIGTON PARK") ~ "BRIGHTON PARK", 
                    str_detect(community, "ILLINOIS MEDICAL DISTRICT") ~ "NEAR WEST SIDE", 
                    str_detect(community, "ROGER’S PARK") ~ "ROGERS PARK",
                    str_detect(community, "GOLD COAST|NAVY PIER") ~ "NEAR NORTH SIDE",
...
Airtable data would suggest that the reports are concentrated along the Northwest Side, followed by the Southwest side. However, this assessment is a very broad one and aggregates data from multiple neighborhood teams. 
...
# produce table visual
all_reports_count %>%
  pivot_wider(names_from = report_status, values_from = n) %>%
  kable()
...
scale_fill_gradient2(
  low = "green", high = "red", name = "Number of Reports") +
...
# V ..................... Stablize the incidents to avoid areas w/ small immigration pop being disproportionately represented  


# nb <- poly2nb(map_data)
#eb <- EBlocal(map_data$false_reports, map_data$foreign_pop, nb = nb, zero.policy = TRUE)

# map_data$rate_smoothed <- eb[, "eb"] * 10000
