...
suburbs_sf <- st_read("Municipality.geojson", quiet = TRUE) %>%
  rename(community = MUNICIPALITY)

# Filter for suburbs around Chicago
# suburbs <- c("Burbank","Oak Park","Forest Park","River Forest",
             #"Maywood","Melrose Park","River Grove","Berwyn","Cicero", "Chicago")

#suburbs_sf <- municipalities %>%
  #filter(community %in% suburbs)
...
suburbs_sf <- st_transform(suburbs_sf, st_crs(chicago_communities))

# Bind our data together 
combined_sf_data_burbs <- bind_rows(evanston_boundaries, suburbs_sf) 
# change to uppercase for matching 
combined_sf_data_burbs$community <- str_to_upper(combined_sf_data_burbs$community)
...
H
```{r, message = FALSE, warning = FALSE, echo = FALSE}
all_reports_count_burbs <- location_data %>% 
group_by(community, report_status) %>%
count() 


# join the counts with evanston data
map_df <- left_join(combined_sf_data_burbs, all_reports_count_burbs, by = "community") %>%
  filter(!is.na(community))
# VI. Map my data ...................................................................
ggplot(map_df) +
  geom_sf(aes(fill = n), color = "black", size = 0.2) +
scale_fill_gradient(low = "green", high = "red", name = "Number of Reports") +
  labs(title = "Figure 1: Reports by Chicago Surburbs (All)") +
  theme_minimal() +
  theme(legend.position = "right")
```
